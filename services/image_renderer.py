...
def render_canvas_in_room(artwork_path, room_filename, wall_color_hex, scale_factor=1.0):
    ...
    try:
        ...
        preview_filename = "preview_rendered.jpg"
        
        # Save to static/images/temp if writable, else use temp directory
        ...
        try:
            ...
            out_path = os.path.join(out_dir, preview_filename)
            room_shadowed.convert('RGB').save(out_path, quality=90)
            return f"/static/images/rooms/{room_filename}"
    except Exception as e:
        print(f"Render error: {e}")
        return f"/static/images/rooms/room_easel.jpg"
